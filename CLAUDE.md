...
| 19 | 给 model 加 `remark` 字段后，前端表格加了列，但三端 API（staff/admin/client）是三个独立端点，改一个漏两个，反复修了 4 次才全部生效 | **给 model 加字段后，必须逐个检查所有相关 API 端点和前端表格是否同步**：先 grep 所有端点 → 逐个确认返回字段 → 逐个确认前端读取 |
