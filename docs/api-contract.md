...
- **POST /staff/loading-manifests/transport-mode**：选的跟现在一样直接返回、不做任何事。除了当前状态，柜子**走过的**步骤里有目标运输方式没有的也不许改（提示先撤销回这些步骤之前）。走过的步骤看：时间表的键、推进账本每一笔的前后状态、开船日期（=运输中）/ 到港日期（=已到港）、柜里货的柜子推进轨迹（`sl_ctn_`，上线前推的老柜子只剩这些；只认货装进这个柜以后写的，先后按记录 id 里的写入时间戳比、放宽 10 分钟）；「目标流程没有的」按两条真流程算。
