...
- **POST /admin/whr-consolidation/customers/price**（改单价，仅超管；员工 `403`）：**恢复**（9-16 到 9-18 之间是 410）。只改传上来的那几档（留空不改）、一档都没传 `400`；**只有 `REPRICE_PLAN_STATUSES`（计划中 / 收货中 / 装柜中）的柜能改**：已完成 `400`、已取消由 `lockPlanAliveById` 拦，前端按钮同步收起来。
  锁序【客户价排队锁 → `lockAgentPriceFloors`（agents `FOR SHARE`，**只拿锁读下限、不判断**）→ 计划行】，**锁到计划之后重读这位客户在柜里那一行**：锁那一刻被移出柜 → `404`「这个客户刚刚被移出这个柜了」（不再 P2025 变 500）；重读之后才用「改完之后的三档」判下限（`assertNotBelowAgentFloors`，不碰库）。
...
- **超管调高代理价**（`/admin/agents/update`）：拦截判断改成查**在跑的柜里当场填的价**（`whr_consolidation_plan_customers`，柜状态在 `REPRICE_PLAN_STATUSES` 里），不再查已经没人写的长期价表；点名的是「哪个客户、哪个柜、哪一档」，最多点名 10 处、超了写「……还有 N 处（共 M 处）」。
  提示给的出口只有两个：**去「集货拼柜(仓库版)」柜详情里「改单价」把柜价改上去**，或者**等这些柜走完（柜里每一票都泰国签收、柜子变成「已完成」）**。⚠️ 别再写「等发运完再调」—— 那是个等不到的状态（同上：没人写 `shipped`）。
  这道闸**故意偏保守**：柜子只要还没走完，客户就可能再往里报新单、按柜里的单价收钱，低于代理价就是每方少收，所以「柜里的单已经全付完」也照样拦。
