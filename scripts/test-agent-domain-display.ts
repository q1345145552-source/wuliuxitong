 ...
 *
 * 盯住两处：
 *  1. 开代理 / 编辑代理弹窗的「专属域名」那一格 —— 填了就把完整登录网址显示出来（跟「前缀」那一格一样），
 *     空着才显示原来那句提示；手抄成 http://Xxx.Example.com/ 按后端 normalizeAgentDomain 同一口径显示干净的。
 *  2. 代理列表「客户登录链接」那一列 —— 前缀和域名各占一行、带「前缀：/域名：」字样，一眼能认出哪个是域名。
 *
...
check("6) 列表：只设了域名（没前缀）也显示域名", () => {
  ...
  assert.ok(!html.includes("前缀："), "没前缀却显示了「前缀：」");
});
