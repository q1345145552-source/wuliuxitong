                      ...
                      {isEditing ? (
                        <>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <select value={draft.receivableCurrency} onChange={(e) => setPrealertEditDrafts((prev) => ({ ...prev, [item.id]: { ...(prev[item.id] ?? buildPrealertDraft(item)), receivableCurrency: e.target.value as "CNY" | "THB" } }))} style={{ border: "1px solid #d1d5db", borderRadius: 6, padding: "6px 4px", fontSize: 12 }}><option value="CNY">人民币</option><option value="THB">泰铢</option></select>
                          </div>
