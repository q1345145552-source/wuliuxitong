 ...
 *     ⚠️ 实测补一句（2026-09-18 在真浏览器里试过）：页面上那几个框是 `type="number"`，
 *     浏览器自己会把「不是合法浮点数」的值清成空串 —— `" 500 "`、`"+500"` 根本到不了这个函数
 *     （手打进去会被吃掉空格变成 `"500"`）。真正被老正则冤枉的是 `".5"`、`"5e2"` 这种。
 *     所以这份对齐的意义是「两边规则只有一套」，不是「救回带空格的粘贴」。
